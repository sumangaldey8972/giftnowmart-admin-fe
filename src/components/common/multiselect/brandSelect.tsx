...
const BrandSelect = ({
    ...
    const fetchBrands = useCallback(async () => {
        try {
            setLoading(true);
            const res = await appClient.get("/api/brand/get", { params: { search, page, limit } });

            if (res.data.status) {
                setBrand(res.data.brands.docs || []);
            }
        } catch (e) {
            console.error(e);
        } finally {
    ...
    const selectedCategory = useMemo(() => {
        return brands.find(c => c._id === value);
    }, [brands, value]);
                    ...
                    >
                        ...
                        <div className="max-h-60 overflow-y-auto scrollbar-thin">
                            {loading ? (
                                ...
                                brands.map((brand, i) => {
                                    const isSelected = value === brand._id;
                                    return (
                                        <motion.button
                                            key={brand._id}
                                            type="button"
